...
COWA是实现数组线程安全一种方式
CopyOnWrite 的意思：就是对一块内存进行修改时，不直接在原有内存块中进行写操作，而是将内存拷贝一份，在新的内存中进行写操作，写完之后，再将原来指向的内存指针指到新的内存，原来的内存就可以被回收。
读的时候没有加锁，写的时候加锁，写入不会阻塞读取操作，然后缺点是在数据量比较大的情况下，要复制一份对内存压力很大
## 6. 线程池怎么实现的
...
1. P：如果sv的值大于零，就给它减1；如果它的值为零，就挂起该进程的执行
2. V：如果有其他进程因等待sv而被挂起，就让它恢复运行，如果没有进程因等待sv而挂起，就给它加1.
...
### 悲观锁和乐观锁
悲观锁是独占锁，只上锁之后别人无法操纵，写入读取都不行，只有释放才可以进行操作：synchronized和ReentrantLock等还有关系型数据库的表锁，行锁
乐观锁是共享锁，更新的时候会判断一下在此期间别人有没有去更新这个数据，可以使用版本号机制和CAS算法实现。乐观锁适用于多读的应用类型，这样可以提高吞吐量
