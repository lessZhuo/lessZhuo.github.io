...
#java数据结构
	...
	* 堆中的划分结构
		...
		* 新生代：一般new对象都会放置在这里
			...
			* eden区：80%生死
			* survivor区：minorgc进入，有两个区域，用复制算法，复制一次+1，次数到了去老年代
	* 对象的回收过程
	...
	* 垃圾回收器（CMS和G1）介绍，分别在哪些代
		...
		* G1
			...
			* 并发：可以与主程序交替进行，并行，几个GC一起发生
			* 分代收集，总体是标记压缩算法，但是对于每个region是复制算法
			* 可以预测的停顿时间模型，可以根据每次停顿时间来控制回收的大小
			* 回收过程：年轻代Eden用尽，
	* 介绍下MinorGC
